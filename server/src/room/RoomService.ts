...
export class RoomService {
    ...
    public removeService(room : string, id : string){
        const theRoom : Room|undefined = this.rooms.find((element)=>{
            if(element.name == room) return element
        });
        if(theRoom !== undefined){
            pos : int = theRoom.players.find(element => {element.})
        }
    }
